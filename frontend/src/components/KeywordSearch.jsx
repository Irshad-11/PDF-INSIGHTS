...
export default function KeywordSearch({ sessionId, fileData }) {
  ...
  const handleResultClick = (result, idx) => {
    ...
    setActiveResult(idx);
    setPreciseHighlightText(result.snippet);   // ← This is what gets strongly highlighted
  };
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] -mx-6">
      ...
      {/* Right panel - unchanged except handleResultClick */}
      <div className="flex-1 pr-6 pl-2 flex flex-col min-h-0">
        <div className="flex gap-2 mb-4 flex-shrink-0">
          ...
          <button onClick={search} disabled={loading || !query.trim()} className={`px-4 py-2.5 rounded-lg text-xs font-medium transition-all flex-shrink-0 ${query.trim() && !loading ? "bg-neutral-100 text-neutral-950 hover:bg-neutral-200" : "bg-neutral-900 border border-neutral-800 text-neutral-700 cursor-not-allowed"}`}>
        ...
        {results && (
          <p className="text-xs text-neutral-500 mb-3 flex-shrink-0">
            <span className="text-neutral-300 font-medium">{results.total}</span> match{results.total !== 1 ? "es" : ""} for "{results.query}"
          </p>
        )}
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-0.5" style={{ scrollbarWidth: "thin", scrollbarColor: "#333 transparent" }}>
          ...
          {!results && !loading && (
            <div className="border border-neutral-600 border-dashed rounded-lg p-10 text-center">
              <Search size={20} className="text-neutral-400 mx-auto mb-2.5" />
              <p className="text-xs text-neutral-500">Search to find matches</p>
